...
void Node::FinalizeSubtree(Page this_page, int level_of_files,
  ...
  for (int j = 0; j < item_num; j++) {
    ...
    if (tmp_nodes->at(j) != nullptr) {
      ...
      if (lsmt->HasHotspot())
        tmp_nodes->at(j)->GetNodeLSMT()->SetHotspot(true);
...
void BplusTree::SetHotspots(std::vector<std::string> hotspots) {
  hotspots_.Clear();
  for (int i = 0; i < hotspots.size(); i += 2) {
    hotspots_.low_keys_.push_back(hotspots[i]);
    hotspots_.up_keys_.push_back(hotspots[i+1]);
    fprintf(stdout, "Hotspot: %s %s\n", hotspots[i].c_str(), hotspots[i+1].c_str());
  }
  GetAdaptStrategy()->SetHotspots(hotspots_);
}
...
void BplusTree::AddWOTCompactionWork() {
  // MutexLock l(&mutex_);
  mutex_.AssertHeld();
  if (!mem_empty_.load() || has_imm_.load() || leveldb_lsmt_->HasHotspot()) {
    return;
  }
  scheduled_flush_num_.fetch_add(1);
...
bool BplusTree::RangeWithinHotspot(const Slice& low, const Slice& up) {
  if (hotspots_.Empty()) return true;
  if (up.empty()) {
    if (hotspots_.WithinHotspot(low)) {
      return true;
    }
    return false;
  }
  return hotspots_.RangeInsideHotspot(low, up);
}
