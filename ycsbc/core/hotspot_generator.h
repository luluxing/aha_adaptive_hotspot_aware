 ...
 public:
  ...
  : dist_(min, max),
    ...
    seed_(seed) {
      ...
      hot_end_ = hot_start_ + hot_interval_ - 1;
  }
  ...
  // The original hotspot starts from the beginning of the data set.
  // The new hotspot starts from the mean of the data distrubtion.
  void ResetHotspot(uint64_t* new_start, uint64_t* new_end) {
    *new_start = (global_min_ + global_max_) / 2;
    *new_end = *new_start + hot_interval_ - 1;
    fprintf(stdout, "Hotspot reset from [%lu, %lu] to [%lu, %lu]\n",
                    hot_start_, hot_end_, *new_start, *new_end);
    hot_start_ = *new_start;
    hot_end_ = *new_end;
  }
...
inline uint64_t HotspotGenerator::Next() {
  ...
  if (hot_op_percent_ == 1.0) {
    ...
    last_int_ = hot_start_ + dist_(generator_) % hot_interval_;
    ...
    if (hot < hot_op_percent_) {
      ...
      auto r = dist_(generator_) % cold_interval_;
      if (r <= hot_start_ - global_min_) {
        last_int_ = global_min_ + r;
      } else {
        last_int_ = global_min_ + hot_interval_ + r;
      }
