...
#ifndef YCSB_C_ZIPFIAN_GENERATOR_SEED_H_
#define YCSB_C_ZIPFIAN_GENERATOR_SEED_H_
...
class ZipfianGeneratorSeed : public Generator<uint64_t> {
 public:
  // 0: uniform; larger number is more skewed
  constexpr static const double kZipfianConst = 0.99;//1.39;
  static const uint64_t kMaxNumItems = (UINT64_MAX >> 24);
  
  ZipfianGeneratorSeed(uint64_t min, uint64_t max, int seed,
  ...
  : num_items_(max - min + 1),
    ...
    n_for_zeta_(0),
    seed_(seed),
    rand_dev_(new utils::RandomDevice(seed)),
    max_val_(max) {
  ...
  // ZipfianGeneratorSeed(uint64_t num_items, int seed=1)
  // : ZipfianGeneratorSeed(0, num_items - 1, kZipfianConst, seed) {}
  ...
  void ResetSeed() {
    rand_dev_ = new utils::RandomDevice(seed_);
  }
...
inline uint64_t ZipfianGeneratorSeed::Next(uint64_t num) {
  ...
  double u = rand_dev_->RandomDouble();
  double uz = u * zeta_n_;
...
inline uint64_t ZipfianGeneratorSeed::Last() {
  std::lock_guard<std::mutex> lock(mutex_);
  return last_value_;
}
...
#endif // YCSB_C_ZIPFIAN_GENERATOR_SEED_H_
