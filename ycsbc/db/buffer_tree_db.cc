...
BufferTreeDB::BufferTreeDB(std::map<std::string, std::string>* props)
  : flush_id_(1),
    interval_(0) {
  is_buffer_tree_ = std::stoi((*props)["is_lsmt"]) == 1 ? false : true;
  is_lsmt_ = false;
  if (is_buffer_tree_)
    is_lsmt_ = std::stoi((*props)["lsmt_level_limit"]) >= 6 ? true : false;
  std::string index_name = is_buffer_tree_ ? "BufferTree" : (is_lsmt_ ? "LSMT" : "AHA-tree");
  ...
  ///////////////////////
  options.adapt_strategy = 1; 
  fprintf(stdout, "Using default adapt_strategy: 1\n");
  ///////////////////////
  ...
  ///////////////////////
  options.proactive_validation = true;
  fprintf(stdout, "Proactive validation enabled\n");
  ///////////////////////
  if (props->find("block_num") != props->end()) {
    options.block_num = std::stoi((*props)["block_num"]);
  } else {
    fprintf(stdout, "Using default block_num: 4\n");
  }
  ...
  tree_ = new BufferTree(options, (*props)["lsmt_path"], flush_id_,
                         is_lsmt_, is_buffer_tree_);
}

void BufferTreeDB::SetHotspots(const std::vector<std::pair<std::string,std::string>> &hotspot) {
  for (auto &p : hotspot) {
...
bool BufferTreeDB::KeyInHotspot(const std::string &key) {
...
int BufferTreeDB::Insert(const std::string &table, const std::string &key,
...
int BufferTreeDB::BatchInsert(const std::string &table,
...
int BufferTreeDB::BatchUpdate(const std::string &table,
                            std::vector<KVPair> &values) {
  if (!tree_insert_) {
...
int BufferTreeDB::Read(const std::string &table, const std::string &key,
...
int BufferTreeDB::Scan(const std::string &table, const std::string &key,
...
int BufferTreeDB::Update(const std::string &table, const std::string &key,
                    std::vector<KVPair> &values) {
  return Insert(table, key, values);
}

int BufferTreeDB::Delete(const std::string &table, const std::string &key){
...
void BufferTreeDB::PrintAllPages() {
  // std::string low = std::string(tmp_key_.length(), '0');
  // std::string up = std::string(tmp_key_.length(), '9');
  // tree_->SetPrintPage();
  // SortedTreeIterator* iter = tree_->NewSortedTreeIterator();
  // std::vector<KVPair> tmp;
...
void BufferTreeDB::RemoveTree(bool concurrent) {
  // tree_->IncreaseNodeSize();
}

void BufferTreeDB::AddTree(bool adapt) {
  if (adapt) {
    tree_->AdaptToRead();
    // if (!hot_queried_start_.empty() && !hot_queried_end_.empty()) {
    //   fprintf(stdout, "Setting hotspot: [%s, %s]\n",
    //           hot_queried_start_.c_str(), hot_queried_end_.c_str());
    //   tree_->SetHotspotRange(hot_queried_start_, hot_queried_end_);
    // }
    if (hotspots_.size() > 0) {
...
bool BufferTreeDB::BGFlushIdle() {
  return tree_->BGFlushIdle();
}

bool BufferTreeDB::BGCompactionIdle() {
  return tree_->BGCompactionIdle();
}

void BufferTreeDB::PrintAllData(std::string lk, std::string uk, std::string filename) {
