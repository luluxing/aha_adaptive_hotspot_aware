...
Status LevelDBLSMT::CompactTree(int* split_num,
  ...
  options.fill_cache = false;
  Iterator* iter;
  if (page_iter == nullptr) {
    iter = NewMergedIterator(options);
  } else {
    Iterator* lsm_iter = NewMergedIterator(options);
    std::vector<Iterator*> iter_vec;
    iter_vec.push_back(lsm_iter);
    iter_vec.push_back(page_iter);
    iter = NewMergingIterator(&icmp_, &iter_vec[0], iter_vec.size());
  }
