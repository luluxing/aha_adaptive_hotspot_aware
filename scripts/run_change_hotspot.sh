...
function run_workload {
  ...
  ../build/ycsbc/ycsb -options options/default_${ind_name}.spec -workload options/default_workload.spec -runningtime ${TOTAL_RUNNING_TIME} -usetime 1 -sequence ${NUM_WORKLOADS} i-0.0-r-0.0-s-${SCAN_RATIO1}-u-${UPDATE_RATIO1} ${SCAN_WORKLOAD_USES_TIME_TO_RUN} ${EXEC_TIME} ${USE_TREE_UPDATE1} ${ADAPT1} ${HOT_OP_RATIO1} ${CHANGE_HOTSPOT1} i-0.0-r-0.0-s-${SCAN_RATIO2}-u-${UPDATE_RATIO2} ${UPDATE_WORKLOAD_USES_TIME_TO_RUN} ${EXEC_OP} ${USE_TREE_UPDATE2} ${ADAPT2} ${HOT_OP_RATIO2} ${CHANGE_HOTSPOT2} i-0.0-r-0.0-s-${SCAN_RATIO1}-u-${UPDATE_RATIO1} ${SCAN_WORKLOAD_USES_TIME_TO_RUN} ${EXEC_TIME} ${USE_TREE_UPDATE1} ${ADAPT1} ${HOT_OP_RATIO1} ${CHANGE_HOTSPOT3}  1> ${ind_name}_$(date +%F_%H_%M).out 2>&1
