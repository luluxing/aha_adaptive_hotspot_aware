...
bool HotOnlyAdapt::HasTargetData(const std::string& lk,
                                 const std::string& uk,
                                LevelDBLSMT* lsmt) {
  return lsmt->HasHotspot(lk, uk);
}
...
void HotOnlyAdapt::CompactRootBufferForFlush(int* level_of_files,
  ...
  do {
    ...
    if (level == -1 && files->size() == 0) {
      ...
      tree_->GetRootLSMT()->SetHotspot(low_hot_key_, up_hot_key_, false);
    ...
    if (FileAcrossPivots(tree_->buffer_manager_, tree_->root_pg_id_, files,
      ...
      files->clear();
      int nl = tree_->GetRootLSMT()->GetFilesInRange(files, low_hot_key_, up_hot_key_);
      // assert(nl == level);
...
void HotOnlyAdapt::CompactNodeBufferForFlush(Node* cur_node, Page page, int* level_of_files,
  ...
  do {
    ...
    if (level == -1 && files->size() == 0) {
      ...
      cur_node->GetNodeLSMT()->SetHotspot(low_hot_key_, up_hot_key_, false);
    ...
    if (FileAcrossPivots(tree_->buffer_manager_, cur_node->pg_id_, files,
      ...
      LevelDBLSMT::RemoveFiles(tree_->env_, tree_->root_lsmt_path_, dead_files);
      files->clear();
      int nl = cur_node->GetNodeLSMT()->GetFilesInRange(files, low_hot_key_, up_hot_key_);
      // assert(nl == level);
      level_mismatch = (nl != level || files->size() == 0);
    }
    *level_of_files = level;
