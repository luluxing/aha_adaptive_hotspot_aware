...
class CoreWorkload {
 public:
  ...
  void ChangeAndResetHotspot() {
    HotspotGenerator* gen = dynamic_cast<HotspotGenerator*>(key_chooser_);
    if (gen == nullptr) {
      fprintf(stdout, "Not a hotspot generator\n");
      std::exit(1);
    }
    uint64_t new_start, new_end;
    gen->ResetHotspot(&new_start, &new_end);
    hot_queried_start_ = BuildKeyName(new_start);
    hot_queried_end_ = BuildKeyName(new_end + fixed_scan_len_);
  }
